...
def load_rows(path: str | Path) -> tuple[list[str], list[int]]:
    ...
    if not isinstance(rows, list) or not rows:
        raise ValueError(
            "Training data must be a non-empty JSON list, mapping, or JSONL file."
        )
...
def train(input_path: str | Path, output_path: str | Path) -> None:
    ...
    print(
        classification_report(
            test_labels, predictions, target_names=["real", "fake"], zero_division=0
        )
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train a supervised fake-post text classifier."
    )
    parser.add_argument(
        "input",
        nargs="?",
        default=str(POST_DATA_PATH),
        help="Labeled .json, .jsonl, or JSON mapping",
    )
